...
class NaiveBayesClassifier(object):
    ...
    def build_bow(self, training_set, V):
        binary_vectors = []
        for sentence in training_set:
            binary_vector = [0] * len(V)
            for word in sentence:
                if word in V:
                    index = V[word]
    ...
    def train(self, training_sentences, training_labels):
        """Trains the model based on the training sentences and training labels.
            Need to estimate the parameter of the probability of each class label and save them in `self.prior`. 
            Need to estimate the parameter of the conditional probability of each unique word given a class `c` and save them in `self.conditional`

        Args:
            training_sentences (List): A list of sentences
            training_labels (List): The list of class labels
        """
        ...
        bagOfWordsVector = self.build_bow(training_set, self.V)
        ...
        for c in all_classes:
            count_c = sum(1 for label in training_labels if label == c)
            self.prior[c] = count_c / N_sentences
        # You don't have to return anything. self.conditional and self.prior will be called in def predict():

        for c in all_classes:
            # Initialize count for each word in class c to 0
            word_counts = {word: 0 for word in self.V}
            total_words = 0
            for i in range(N_sentences):
                if training_labels[i] == c:
                    # Increment count for each word in sentence i
                    for word in training_set[i]:
                        word_counts[word] += 1
                        total_words += 1
            # Compute conditional probabilities for each word in class c
            self.conditional[c] = {word: (count + 1) / (total_words + len(self.V)) for word, count in word_counts.items()}
    ...
    def predict(self, test_sentence):
        """This function finds the log probability for each class label for each given `test_sentence` and stores them in the `label_probability` variable.

        Args:
            test_sentence (_type_): _description_

        Returns:
            int: Returns a label that is Positive(+1), Negative(-1), Neutral (0).
        """
